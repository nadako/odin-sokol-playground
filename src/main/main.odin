...
frame :: proc "c" () {
	...
	if mod_time_err == nil && game.mod_time != mod_time {
		log.debug("Game library modification detected")
		changed = true
	}
	...
	if changed {
		...
		reload = game.should_reload()
		restart = game.should_restart()
	}

	if reload || restart {
		new_game, new_game_ok := load_game_dll()
		if new_game_ok {
			log.debug("Reloading game library")
			old_game := game
			game = new_game

			restart ||= game.get_checksum() != old_game.get_checksum()
