...
tools\sokol-shdc -i src/game/shader.glsl -o src/game/shader.odin -l hlsl5 -f sokol_odin
if %ERRORLEVEL% NEQ 0 exit /b 1

odin build src/game -debug -define:SOKOL_DLL=true -build-mode:dll -out:bin/hot_reload/game.dll -pdb-name:bin/hot_reload/game_%PDB_NUMBER%.pdb
if %ERRORLEVEL% NEQ 0 exit /b 1
